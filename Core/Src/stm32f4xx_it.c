...
void HardFault_Handler(void)
{
  ...
  /* Try to send "FAULT" on both UARTs, then blink LED */
  {
    extern UART_HandleTypeDef huart1;
    extern UART_HandleTypeDef huart3;
    uint8_t msg[] = "FAULT\r\n";
    HAL_UART_Transmit(&huart1, msg, sizeof(msg) - 1, 500);
    HAL_UART_Transmit(&huart3, msg, sizeof(msg) - 1, 500);
  }
  while (1)
  {
    HAL_GPIO_TogglePin(GPIOE, GPIO_PIN_10);
    for (volatile uint32_t i = 0; i < 10000000; i++);  /* ~60ms at 168MHz = ~8Hz blink */
  }
...
void TIM7_IRQHandler(void)
{
  /* USER CODE BEGIN TIM7_IRQn 0 */
    ...
    if(__HAL_TIM_GET_FLAG(&htim7, TIM_FLAG_UPDATE) != RESET) {
		...
		encoder_update(motors[0], 0.005, __HAL_TIM_GET_COUNTER(&htim5));
		encoder_update(motors[1], 0.005, __HAL_TIM_GET_COUNTER(&htim2));
		encoder_update(motors[2], 0.005, __HAL_TIM_GET_COUNTER(&htim4));
		encoder_update(motors[3], 0.005, __HAL_TIM_GET_COUNTER(&htim3));
		/* All 4 motors have physically inverted encoder wiring:
		 * positive PWM causes counter to count DOWN → rps is negative.
		 * Negate rps/tps so the PID sees correct sign (negative feedback). */
		motors[0]->rps = -motors[0]->rps;
		motors[0]->tps = -motors[0]->tps;
		motors[1]->rps = -motors[1]->rps;
		motors[1]->tps = -motors[1]->tps;
		motors[2]->rps = -motors[2]->rps;
		motors[2]->tps = -motors[2]->tps;
		motors[3]->rps = -motors[3]->rps;
		motors[3]->tps = -motors[3]->tps;
		for(int i = 0; i < 4; ++i) {
			encoder_motor_control(motors[i], 0.005);
		}
    }
