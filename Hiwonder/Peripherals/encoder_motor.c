...
void encoder_motor_control(EncoderMotorObjectTypeDef *self, float period)
{
    ...
    /* Anti-windup: when motor is stalled (rps≈0), cap accumulated pulse so the
     * motor doesn't lunge at full speed the moment the obstruction is removed. */
    if (self->rps > -0.15f && self->rps < 0.15f) {
        if (pulse >  400.0f) pulse =  400.0f;
        if (pulse < -400.0f) pulse = -400.0f;
    }

    self->set_pulse(self, pulse > -50 && pulse < 50 ? 0 : pulse); /* 设置新的PWM值且限制 PWM 的最小值, PWM过小电机只会发出嗡嗡声而不动 */
