...
//	printf("%d\r\n", adc_value[0]);
    ...
    /* Transmit battery voltage over USB serial at ~1 Hz */
    {
        static int batt_report_count = 0;
        batt_report_count++;
        if(batt_report_count >= (int)(1000 / BATTERY_TASK_PERIOD)) {
            batt_report_count = 0;
            uint16_t mv = (uint16_t)battery_volt;
            uint8_t batt_data[3] = {0x04, (uint8_t)(mv & 0xFF), (uint8_t)(mv >> 8)};
            packet_transmit(&packet_controller, PACKET_FUNC_SYS, batt_data, sizeof(batt_data));
        }
    }
